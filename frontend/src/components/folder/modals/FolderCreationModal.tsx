      ...
      <div className="max-h-48 space-y-2 overflow-y-auto">
        {items.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`flex cursor-pointer items-center gap-3 rounded-lg border px-2 py-2 text-left transition-colors ${
                isSelected
        ...
        >
          <div className="flex-1 space-y-6 overflow-y-auto p-6">
            ...
            <div>
                ...
                <div className="mb-4 rounded-lg bg-blue-50 p-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedItemsWithDetails.map((item) => (
                      <span
                        key={item.id}
                        className="inline-flex items-center gap-1 rounded border border-black/10 bg-white px-2 py-1 text-xs text-gray-900"
                      >
                        {item.type === "predictor" ? (
                          <BrainCircuit className="h-3.5 w-3.5 text-gray-500" />
                        ) : (
              ...
              {/* Item Type Tabs */}
              <div className="mb-4 flex border-b border-black/10 text-sm font-medium">
                <button
                  type="button"
          ...
          {/* Footer */}
          <div className="flex justify-end gap-3 border-t border-black/10 bg-gray-50 p-6">
            <button
              type="button"
              onClick={onClose}
              disabled={isLoading}
              className="rounded-md border border-black/10 bg-white px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
            >
              Cancel
            </button>
            <button
